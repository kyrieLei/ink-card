                               ...
                               unsigned char *Dispbuff)
{
    ...
    for (i = 0; i < YSize; i++)
    {
        for (j = 0; j < XSize; j++)
        {
//            add inversion here to correct the colours
//            SpiWrite(*Dispbuff);
            SpiWrite(255 - *Dispbuff);
            Dispbuff++;
        }
    }
...
//    EpdW21SetRamPointer(0x00, (yDot - 1) % 256, (yDot - 1) / 256);    // set ram
    if (Label == 0)
    {
        // writes white to the display
        EpdW21WriteDispRamMono(xDot, yDot, 0xff);    // white
    } else
    {
        // writes display buffer to the display
        // ! add inversion in this function below
